...
import {
    Container, Row, Col, Card, InputGroup, FormControl, Button, Table,} from "react-bootstrap";
import {useMutation, useQuery, useQueryClient} from "react-query";
import {addConversation, getProviderId, getConversationHistory, transferCustomer, getReadyProviders} from "../api";
...
function Chat() {
    ...
    const [transferMessageRef, changeTransferMessageRef] = useState(React.createRef());
        ...
        getReadyProviders().then((data) => {
            ...
            if (transferMessageRef)
                if (data.length < 1) {
                    ...
                    transferCustomer(receiverId, providerId.provider_id).then(data=>{
